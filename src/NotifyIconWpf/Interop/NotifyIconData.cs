...
namespace Hardcodet.Wpf.TaskbarNotification.Interop
{
	...
	public struct NotifyIconData
	{
		...
		/// <returns></returns>
		public static NotifyIconData CreateDefault(IntPtr handle, Icon iconData, Guid appID)
		{
			...
			data.IconHandle = iconData == null ? IntPtr.Zero : iconData.Handle;
			...
			//set flags
			data.Flags = IconDataMembers.Message | IconDataMembers.Icon | IconDataMembers.Tip;

			//reset strings
			data.ToolTipText = data.BalloonText = data.BalloonTitle = String.Empty;

			if (Environment.OSVersion.Version >= new Version(6, 1) && appID != Guid.Empty)
			{
				data.GuidItem = appID;
				data.Flags |= IconDataMembers.Guid;
			}
